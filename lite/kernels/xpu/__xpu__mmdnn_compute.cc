...
void grnn_layout(int batch, const std::vector<int>& offset, std::vector<int>& new_offset,
...
class id_info{
 ...
 public:
  ...
  void init(int UB_batch, int UB_seqlen) {
    ...
    int total_size = ub_lod_64_size + ub_lod_32_size + ub_new_offset_32_size + ub_idx_sorted_32_size;

    // TODO(miaotianxiang): use l3?
    l3_buffer_guard = TargetWrapperXPU::MallocScratchPad(total_size, false);
    l3_buffer = (char*)l3_buffer_guard->addr_;
  ...
  void update(lite::Tensor* _id0, lite::Tensor* _id1) {
    ...
    memcpy(cpu_buffer + offset, id0_lod.data(), id0_lod.size() * sizeof(int64_t));
    ...
    xpu_memcpy(l3_buffer, cpu_buffer, offset, XPUMemcpyKind::XPU_HOST_TO_DEVICE);
  }
};
...
class fc_op {
 ...
 public:
  ...
  void init(const int16_t* _weight, float _weight_max, const float* _bias,
      int _n, int _k, xdnn::Activation_t::act_enum _act_type) {
    n = _n;
    k = _k;
    act_type = _act_type;
  ...
  void init(lite::Tensor* _weight, float _weight_max, lite::Tensor* _bias,
      int _n, int _k, xdnn::Activation_t::act_enum _act_type) {
  ...
  void infer(xdnn::Context* ctx, const float* in, int m, float* out,
...
class grnn_op {
 ...
 public:
  ...
  void init(lite::Tensor* _wh, const std::vector<float>& _wh_maxs,
      lite::Tensor* _wi, const std::vector<float>& _wi_maxs,
      int _cap_e, int _cap_h, int _max_cap_l) {
    cap_e = _cap_e;
    cap_h = _cap_h;
    ...
    fc_e2h0.init(dense_e2h, _wi_maxs[0], nullptr, cap_h, cap_e, xdnn::Activation_t::LINEAR);
    fc_e2h1.init(dense_e2h + cap_e * cap_h, _wi_maxs[1], nullptr, cap_h, cap_e, xdnn::Activation_t::LINEAR);
    fc_e2h2.init(dense_e2h + cap_e * cap_h * 2, _wi_maxs[2], nullptr, cap_h, cap_e, xdnn::Activation_t::LINEAR);
  ...
  void infer(xdnn::Context* ctx, const id_info& sentense, const float* in, float* out,
    ...
    xdnn::search_grnn<float, int16_t>(ctx, cap_l, cap_h, cap_e,
        max_width, sentense.new_offset_32, fc_e2h_out, dense_h2h, gru_out,
        dense_h2h_max[0], dense_h2h_max[1], dense_h2h_max[2]);
...
public:
  void init(lite::Tensor* _att_fc_w, float _att_fc_w_max, lite::Tensor* _att_fc_b,
      int _dim, int UB_batch, int UB_seqlen) {
    dim = _dim;
    alpha0 = 0.0883883461356163f; // TODO
    alpha1 = 1.0f;

    seqfc.init(_att_fc_w, _att_fc_w_max, _att_fc_b, dim, dim, xdnn::Activation_t::LINEAR);
  ...
  void infer(xdnn::Context* ctx, const id_info& sentense, const float* input, float* pool_out,
    ...
    xdnn::search_noaligned_mat_mul(ctx, 0, 1, batch, lod_32, max_width, dim,
        alpha0, input, seqfc_out, batchgemm0_out);
    ...
    xdnn::search_noaligned_mat_mul(ctx, 0, 0, batch, lod_32, max_width, dim,
        alpha1, seq_softmax_out, input, batchgemm1_out);
...
class match_conv_topk {
 ...
 public:
  ...
  void init(lite::Tensor* _input_w, float _input_w_max,
      lite::Tensor* _conv_w, float _conv_w_max,
    ...
    xpu_memcpy(topks_xpu, topks.data(),
  ...
  void infer(xdnn::Context* ctx, lite::Tensor* left, lite::Tensor* right, lite::Tensor* out,
      float* l3_buffer = nullptr, int l3_size = 0) {
    ...
    xpu_memcpy(left_lod_32, left_lod_32_cpu.data(),
    ...
    xpu_memcpy(right_lod_32, right_lod_32_cpu.data(),
    ...
    xpu_memcpy(match_lod_32, lod_match.data(), lod_match.size() * sizeof(int),
        XPUMemcpyKind::XPU_HOST_TO_DEVICE);
    xpu_memcpy(conv_lod_32, lod_conv.data(), lod_conv.size() * sizeof(int),
        XPUMemcpyKind::XPU_HOST_TO_DEVICE);
    xpu_memcpy(topk_offset_32, lod_topk.data(), lod_topk.size() * sizeof(int),
        XPUMemcpyKind::XPU_HOST_TO_DEVICE);
    ...
    xdnn::search_varconv<float, int16_t>(ctx, batch, dim_t, out_channel, 5, 5, 1, 1, xwy_out, conv_weight,
        right_lod_32, left_lod_32, conv_out, conv_weight_max, xdnn::Activation_t::RELU); // TODO
...
class bid_emb_grnn_att {
 ...
 public:
  ...
  void init(lite::Tensor* _table,
      lite::Tensor* _fw_wh, const std::vector<float>& _fw_wh_maxs,
      lite::Tensor* _fw_wi, const std::vector<float>& _fw_wi_maxs,
      lite::Tensor* _rv_wh, const std::vector<float>& _rv_wh_maxs,
      lite::Tensor* _rv_wi, const std::vector<float>& _rv_wi_maxs,
      lite::Tensor* _att_fc_w, float _att_fc_w_max, lite::Tensor* _att_fc_b,
    ...
    bi_fw.init(_fw_wh, _fw_wh_maxs, _fw_wi, _fw_wi_maxs, emb_dim, cap_h, max_cap_l);
    bi_rv.init(_rv_wh, _rv_wh_maxs, _rv_wi, _rv_wi_maxs, emb_dim, cap_h, max_cap_l);
    att.init(_att_fc_w, _att_fc_w_max, _att_fc_b, 2 * cap_h, UB_batch, UB_seqlen);

    hbm_buffer_guard = TargetWrapperXPU::MallocScratchPad(4 * max_cap_l * cap_h * sizeof(float), false);
    hbm_buffer = (float*)hbm_buffer_guard->addr_;
  }
...
class emb_att {
 ...
 public:
  ...
  void infer(xdnn::Context* ctx, int batch, const id_info& sentense,
      lite::Tensor* att_pool_out, lite::Tensor* emb_fw_out,
      float* l3_buffer = nullptr, int l3_size = 0) {
    emb_fw = emb_fw_out->mutable_data<float>(TARGET(kXPU));
    att_out = att_pool_out->mutable_data<float>(TARGET(kXPU));
...
class merge_all {
 ...
 public:
  void init(lite::Tensor* grnn_fw_wh, std::vector<float> grnn_fw_wh_maxs,
    ...
    coverage_fw.init(grnn_fw_wh, grnn_fw_wh_maxs,
        grnn_fw_wi, grnn_fw_wi_maxs,
        cap_e, cap_h, max_cap_l);
    coverage_rv.init(grnn_rv_wh, grnn_rv_wh_maxs,
  ...
  void infer(xdnn::Context* ctx, const id_info& sentense,
      const std::vector<lite::Tensor*> concat_2in1_x,
      const std::vector<lite::Tensor*> concat_7in1_x,
      lite::Tensor* out, float* l3_buffer = nullptr, int l3_size = 0) {
    int batch = sentense.batch;
    int cap_l = sentense.seqlen_sum;
    ...
    const int concat_widths[] = {(int)concat_2in1_x[0]->dims()[1], (int)concat_2in1_x[1]->dims()[1]};
    ...
    xdnn::sequence_reverse(ctx, batch, sentense.lod_32, cap_e, topk_concat_out_fw, topk_concat_out_rv);
    ...
    const int concat_widths_fc0[] = {
      (int)concat_7in1_x[0]->dims()[1],
      (int)concat_7in1_x[1]->dims()[1],
      (int)concat_7in1_x[2]->dims()[1],
      (int)concat_7in1_x[3]->dims()[1],
      (int)concat_7in1_x[4]->dims()[1],
      (int)concat_7in1_x[5]->dims()[1],
      (int)concat_7in1_x[6]->dims()[1],
    };
...
void XPUMMDNNBidEmbGrnnAttCompute::PrepareForRun() {
  ...
  compound.init(param.emb_tbl,
      param.grnn_fw_wh, param.grnn_fw_wh_maxs,
      param.grnn_fw_wi, param.grnn_fw_wi_maxs,
      param.grnn_rv_wh, param.grnn_rv_wh_maxs,
      param.grnn_rv_wi, param.grnn_rv_wi_maxs,
      param.att_fc_w, param.att_fc_w_max, param.att_fc_b,
      UB_batch, UB_seqlen);
}

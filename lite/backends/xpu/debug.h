...
template <typename T>
void dump_cpu_mem(const T* ptr, size_t len, const std::string& comment = "", size_t stride = 1, size_t item_per_line = 30) {
  ...
  printf("------------------------------ [%s] len=%zd stride=%zd sum=%f BEGIN ------------------------------\n",
  ...
  for (size_t i = 0; i < nline; ++i) {
    ...
    printf("line[%04zd] -- ", i);
    for (size_t ii = line_begin; (ii < line_end) && (ii < after_stride_len); ++ii) {
      if (std::is_same<T, float>::value) {
        printf("%.6f, ", (float)after_stride[ii]);
      } else if (std::is_same<T, int16_t>::value) {
        printf("%d ", (int)after_stride[ii]);
      } else {
...
template <typename T>
void dump_xpu_mem(const T* ptr, size_t len, const std::string& comment = "", size_t stride = 1, size_t item_per_line = 30) {
  size_t after_stride_len = (len + stride - 1) / stride;
  std::unique_ptr<T[]> cpu_mem(new T[len]);
  xpu_memcpy(cpu_mem.get(), ptr, len * sizeof(T), XPUMemcpyKind::XPU_DEVICE_TO_HOST);
  std::unique_ptr<T[]> after_stride(new T[after_stride_len]);
  for (size_t i = 0; i < after_stride_len; ++i) {

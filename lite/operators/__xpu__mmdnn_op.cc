...
bool XPUMMDNNBidEmbGrnnAttOp::AttachImpl(const cpp::OpDesc& op_desc,
  ...
  param_.id1 = scope->FindVar(op_desc.Input("id1").front())
                   ->GetMutable<lite::Tensor>();
  ...
  param_.att_pool_out = scope->FindVar(op_desc.Output("att_pool_out").front())
                   ->GetMutable<lite::Tensor>();
  param_.concat_3in1_out = scope->FindVar(op_desc.Output("concat_3in1_out").front())
                   ->GetMutable<lite::Tensor>();
...
bool XPUMMDNNMergeAllOp::CheckShape() const {
  return true;
}
...
bool XPUMMDNNMergeAllOp::AttachImpl(const cpp::OpDesc& op_desc,
  ...
  param_.out = scope->FindVar(op_desc.Output("out").front())
                   ->GetMutable<lite::Tensor>();
